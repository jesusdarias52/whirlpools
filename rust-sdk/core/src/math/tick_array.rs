...
use super::get_initializable_tick_index;

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TickArraySequence {
    pub tick_arrays: Vec<Option<TickArrayFacade>>,
    pub tick_spacing: u16,
    /// All initialized ticks within [start_index, end_index], sorted ascending by tick_index.
    initialized_ticks_asc: Vec<(i32, TickFacade)>,
}
...
impl TickArraySequence {
    pub fn new(
        ...
        // Compute sequence bounds (same logic as start_index / end_index, but inline since
        // self doesn't exist yet).
        let seq_start = start_tick_index(&tick_arrays[0]).max(MIN_TICK_INDEX);
        let mut last_valid_start = seq_start;
        for ta in &tick_arrays {
            if start_tick_index(ta) != <i32>::MAX {
                last_valid_start = start_tick_index(ta);
            }
        }
        let seq_end = (last_valid_start + TICK_ARRAY_SIZE as i32 * tick_spacing as i32 - 1)
            .min(MAX_TICK_INDEX);

        // Build the cache: collect all initialized ticks within [seq_start, seq_end], sorted ASC.
        let mut initialized_ticks_asc: Vec<(i32, TickFacade)> = tick_arrays
            .iter()
            .filter_map(|ta| ta.as_ref())
            .flat_map(|ta| {
                ta.ticks.iter().enumerate().filter_map(|(i, tick)| {
                    if tick.initialized {
                        let idx = ta.start_tick_index + i as i32 * tick_spacing as i32;
                        if idx >= seq_start && idx <= seq_end {
                            Some((idx, *tick))
                        } else {
                            None
                        }
            ...
            .collect();
        initialized_ticks_asc.sort_by_key(|(idx, _)| *idx);

        Ok(Self {
            tick_arrays,
            tick_spacing,
            initialized_ticks_asc,
        })
    }
    ...
    pub fn next_initialized_tick(
        ...
        // Find the first initialized tick with index strictly greater than tick_index.
        let pos = self
            .initialized_ticks_asc
            .partition_point(|(idx, _)| *idx <= tick_index);
        if pos < self.initialized_ticks_asc.len() {
            let (found_index, ref tick) = self.initialized_ticks_asc[pos];
            Ok((Some(tick), found_index))
        } else {
            // No initialized tick found ahead — return end sentinel.
            Ok((None, array_end_index))
        }
    }
    ...
    pub fn prev_initialized_tick(
        ...
        let initializable_index =
            get_initializable_tick_index(tick_index, self.tick_spacing, Some(false));
        // Find the last initialized tick with index <= initializable_index.
        // partition_point in asc gives us the first element > initializable_index,
        // so pos-1 (if pos > 0) is the last element <= initializable_index.
        let pos = self
            .initialized_ticks_asc
            .partition_point(|(idx, _)| *idx <= initializable_index);
        if pos > 0 {
            let (found_index, ref tick) = self.initialized_ticks_asc[pos - 1];
            Ok((Some(tick), found_index))
        } else {
            // No initialized tick found behind — return start sentinel.
            Ok((None, array_start_index))
        }
    }
}
...
mod tests {
    ...
    fn test_sequence_with_one_tick_array(
        ...
        TickArraySequence::new(Vec::from([Some(one), None, None, None, None]), tick_spacing).unwrap()
    }
    ...
    fn test_sequence(
        ...
        TickArraySequence::new(
            Vec::from([Some(one), Some(two), Some(three), None, None]),
            tick_spacing,
        )
        .unwrap()
    }
